...
>>>>>>> parent of 343b58c... try_4 saved images
            ...
            else
            {
                ...
                if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
                {
                    ...
                    SaveLocation = Server.MapPath("~/images/")  + fileName;

                    try
                    {
                        FileUpload1.PostedFile.SaveAs(SaveLocation);
                        fileName2 = Global.uploadImage(SaveLocation, produc + "_" + DateTime.Now.ToString().Replace("/", "_").Replace(":", "_").Replace(" ", "_"));
                        Response.Write("The file has been uploaded.");
                    }
                    catch (Exception ex)
                    {
                ...
                NewBike.AddNewBike(NewBike.getMaxId(), catego, sub_catego, produc, siz, Specification.Text, color, Convert.ToDouble(RetailPrice.Value), Convert.ToDouble(RegularPrice.Value), Convert.ToDouble(ClubPrice.Value), Convert.ToInt32(Quantity.Value), fileName, Model.Value);
